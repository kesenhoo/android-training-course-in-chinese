...
为了有一个单一控制点用来限制CPU或网络资Runnable类型，你可能想有一个能管理每一个线程的线程池，且每个线程都会是单个实例。比如，你可以把这个作为一部分添加到你的全局变量的声明中去：
```java
...
一个Handler允许你的APP安全地调用UI对象（例如View对象）的方法。大多数UI对象只能从UI线程安全地被修改。这个方法将会在<a href="performance/multi-threads/communicate-ui.html" target="_blank">与UI线程进行通信(Communicate with the UI Thread)</a>这一课中进行详细的描述。例如：
```java
private PhotoManager() {
    ...
