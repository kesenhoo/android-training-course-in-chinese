...
编程者需要“不过度分配”局部引用。在实际操作中这意味着如果你正在创建大量的局部引用，或许是通过对象数组，你应该使用DeleteLocalRef手动地释放它们，而不是寄希望JNI来为你做这些。实现上只需要预留16个局部引用的空间，所以如果你需要更多，要么你删掉以前的，要么使用EnsureLocalCapacity/PushLocalFrame来预留更多。
...
Java编程语言使用UTF-16格式。为了便利，JNI也提供了支持变形UTF-8的方法。这种变形编码对于C代码是非常有用的，因为它将\u0000编码成0xc0 0x80，而不是0x00。最惬意的事情是你能在具有C风格的以\0结束的字符串上计数，同时适用于标准的libc字符串函数。不好的一面是你不能传入随意的UTF-8数据到JNI函数而还期望它正常工作。

如果可能，操作UTF-16字符串通常更快些。安卓当前不需要GetStringChars做出拷贝，而GetStringUTFChars需要一次分配并且转换为UTF-8格式。注意**UTF-16字符串不是以零终止字符串**，\u0000是被允许的，所以你需要和jchar指针处理一样继续计算字符串的长度。

**不要忘记Release你Get的字符串**。这些字符串函数返回jchar*或者jbyte*，都是指向基本数据类型的C格式的指针而不是局部引用。它们在Release调用之前都保证有效，这意味着当原生方法返回时它们并不主动释放。

**传入NewStringUTF函数的数据必须是变形UTF-8格式**。从文件或者网络流中读取出的字符数据，没有过滤直接使用NewStringUTF处理是一种常见的错误。除非你确定数据是7位的ASCII格式，否则你需要剔除超出7位ASCII编码范围（high-ASCII）的字符或者将它们转换为对应的变形UTF-8格式。如果你没那样做，UTF-16的转换结果可能不会是你想要的结果。JNI扩展检查将会扫描字符串，然后警告你那些无效的数据，但是它们将不会发现所有潜在的风险。
